...
It's easy to test the behaviour of the parsing of an hand, or a single card, in isolation. Using the single class version of the parser, we might feel tempted to access the private methods (e.g. `parseHand`, or `parseCard`) due _Insufficient Access_ [^synergy_between_testability_and_design] on the parser class.

### 3. Comprehensibility

I don't have to puzzle my mind scanning though a lot of lines of code looking for the part of the code I am interested in.
